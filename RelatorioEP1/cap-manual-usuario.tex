...
\begin{enumerate}
\item Login
\item Registro e envio de incidente
\item Lista de incidentes não anviados
\end{enumerate}

%Inserir telas do aplicativo de usuário
\subsection{Login}
A interface inicial do sistema do Usuário é a tela de Login apresentada na Figura\ref{fig:Ouvidoria-Login}. 

O usuário deste aplicativo pode ser qualquer pessoa que possua um número USP e deseja registrar algum incidente para que os responsávei o tratem.

Para que o login seja realizado com sucesso, o usuário precisa informar o mesmo login e senha cadastrados na rede social STOA. Caso ainda não possua cadastro na rede social, este pode ser realizado por meio do link:
...
\begin{figure}[!h]
  \centering
  \includegraphics[width=.40\textwidth]{Usuario-Login} 
  \caption{Interface de login para o aplicativo do Usuário}
  \label{fig:Usuario-Login} 
\end{figure}

\subsection{Registro e envio de incidente}
...
\begin{enumerate}
\item Categoria - Basta iniciar a digitação que será exibida uma lista de categorias pré-existentes.
\item Descrição do incidente - Preencher com uma breve descrição o incidente.
\item Salvar - Botão no canto superior direito ao qual realiza o envio dos dados do incidente ao servidor no caso de haver conexão de dados, do contrário, salva o incidente no banco de dados local para envio posterior, quando houver conexão.
\end{enumerate}

A Figura\ref{fig:Usuario-Incidente_Novo} apresenta a interface para o registo e envio de incidentes.

\begin{figure}[!h]
  \centering
  \includegraphics[width=.40\textwidth]{Usuario-Incidente_Novo} 
  \caption{Interface de apresentação para inserir um novo incidente}
  \label{fig:Usuario-Incidente_Novo} 
\end{figure}

\subsection{Lista de incidentes não anviados}
...
A Figura\ref{fig:Usuario-ListaIncidentesPendentes} apresenta a interface com a lista de incidentes pendentes de envio.

\newpage
\begin{figure}[!h]
  \centering
  \includegraphics[width=.40\textwidth]{Usuario-ListaIncidentesPendentes} 
  \caption{Interface de apresentação da lista de incidentes pendentes de envio}
  \label{fig:Usuario-Incidente_Novo} 
\end{figure}
...
O usuário informa uma palavra-chave presente na descrição do incidente cadastrado e o aplicativo apresentará uma lista de todos os incidentes encontrados que possuam a palavra-chave informada em sua descrição. O resultado da busca será apresentado na lista de incidentes, conforme Figura\ref{fig:Ouvidoria-ResultadoDaBusca}.
...
Ao clicar sobre o status atual na tela para Apresentação de detalhes de cada incidente registrado, será exibida uma janela pop-up onde será possível escolher um novo status para o incidente registrado. A Figura\ref{fig:Ouvidoria-AtualizacaoDoStatusDoIncidente} exibe a tela para atualização do status de um incidente selecionado.
...
O botão para exibição dos incidentes no mapa (ícone de mapa) permite visualizar no mapa todos os incidentes cadastrados com coordenadas GPS. Marcadores indicam o local do incidente no mapa da USP. A Figura\ref{fig:Ouvidoria-ApresentacaoDeIncidentesNoMapa} apresenta a interface para visualização de incidentes no mapa.
